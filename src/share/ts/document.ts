...
import { InMemory } from './data_backend/data_backend';
import {
...
export default class Document extends EventEmitter {
  ...
  public name: string;
  public root: Path;
  public useCacheOnly: boolean;

  constructor(store: DocumentStore, name = '', useCacheOnly: boolean = false) {
    super();
  ...
  public async getInfo(row: Row, ignoreCache: boolean = false): Promise<CachedRowInfo> {
    ...
    const finalCollapsed = (ignoreCache && cached) ? cached.collapsed : collapsed;
    const info: RowInfo = {
      line,
  ...
  public async setLine(row: Row, line: Line) {
    ...
    await this.emitAsync('lineSaved', { row, line });
  }
  ...
  public async getAncestryPaths(row: Row): Promise<Array<Row>> {
    const parents = await this._getParents(row);
    if (parents[0] !== Path.rootRow()) {
      const ancestryPaths = await this.getAncestryPaths(parents[0]);
      ancestryPaths.push(parents[0]);
  ...
  public async reload(serialized_rows: Array<SerializedBlock>) {
    ...
    await this.store.setParents(0, []);
    await this.load(serialized_rows);
  }
}
